...
- Папка `game`: содержит всё, что связано с самой игрой. Это:
    - Папка `core`: содержит главные классы `Game`, `Loader`, `World`, `ItemsDB`, а также `Achievements` (в будущем).
    - Папка `entity`: содержит основной класс `Entity`, который наследуют `Player` и `Mob`, класс `Effect`, класс `Attack` вместе с его детекторами  (`RayDetector`, `AreaDetector`, ...) и распространёнными формами (`ColorAttack`, ...). Также содержит в себе папку `effects` со всевозможными эффекты, в отдельных подпапках.
    ...
    - Папка `shared` содержит различные ресурсы, используемые по всей игре.
    ...
    - Папка `solo`: содержит в своих подпапках все ресурсы для одиночной игры. Чего нет в мультиплеере, всё идёт сюда.
        - Папка `training` содержит в себе все файлы, связанные с тренировкой.
        - Папка `challenges` содержит в себе папки с различными испытания в одиночной игре и папку `common` с различными ресурсами, общими между этими испытаниями.
        - Папка `story` содержит в себе всё связанное с сюжетной игрой.
...
- Папка `main`: содержит классы `Main`, `Globals` и `Utils`, а также прочие ресурсы, которые используются при запуске игры.
...
- Папка `images`: оригиналы различных изображений в формате программ, где они были созданы.
...
Сохранения в игре реализованы через класс `Globals`, а точнее через его методы `set_*()` и `get_*()`. Также для удобства хранения есть также методы `set_controls_*()` и `get_controls_*()` (для хранения настроек управления) и методы `set_setting_*()` и `get_setting_*()` (для хранения остальных настроек).
...
- Индекс -2: объекты между полом и стенами. Снаряды и игроки находятся над ними (забор, пень).
...
- `Damageable`: слой для объектов, через которые можно ходить, но нельзя стрелять. С ними сталкиваются атаки, но не сущности.
- `Other`: слой для прочих объектов.
...
- Канал 2: используется чатом и эмоциями.
