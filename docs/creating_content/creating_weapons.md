...
2. В этой папке создайте ресурс типа `WeaponData`, назовите именем Вашего оружия (всё ещё в *snake_case*) и заполните в нём информацию:
    ...
    - Во вкладке `About`:
        ...
        - В `Description` полностью опишите оружие по плану ниже, отделяя пункты друг от друга пустой строкой. Рекомендуется выделять числа с помощью *BBCode*: связанное с уроном - красным (`[color=red]`), с боеприпасами - синим (`[color=blue]`), особенностями - голубым (`[color=deep_sky_blue]`). Максимальная точность дробных чисел - десятые доли.
            ...
            - Текстовое описание
        - В `Speed Multiplier When Current` укажите то же значение, что в одноименном поле у самого оружия.
...
- Перед рисованием ознакомьтесь с [`drawing.md`](./drawing.md), а также про ещё несколько моментов:
    ...
    - После того как закончите рисовать, сохраните файл в формате в куда нибудь в папку `images`, чтобы не потерять оригинал.
...
- После вызова функции воспроизведения анимации экипировки используйте `_anim.advance(0.0)`, чтобы сразу же переместить оружие в стартовое положение анимации.
- Рекомендуется хранить визуальную составляющую оружия и его анимации в отдельных файлах в подпапке `visual`.
- Можно сохранять состояние оружия в `Player.persistent_data`, желательно префиксовать ключ словаря ID оружия.
