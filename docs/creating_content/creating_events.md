...
6. Нарисуйте обложку события в разрешении 784 на 160. Сохраните её как `game/events/<имя события>/images/image.png`.
7. Создайте ресурс типа `EventData` с именем `<имя события>.tres` в папке Вашего события. Заполните название и краткое описание. В поле `Scene Path` перетащите сцену Вашего события, а в `Image Path` - `image.png`. Также отредактируйте (если нужно) `Min Players`, `Max Players` и `Team Event`. Подробности смотрите в документации к этим свойствам.
8. В свойстве `Description` в ресурсе `EventData` опишите суть события, его цели, участников и условия окончания.
9. Откройте `game/core/items_db.tres` и в свойство `Events` добавьте `*.tres` файл нового события.
10. Готово! Теперь событие доступно для выбора в игре. Однако, сперва нужно создать хотя бы одну карту - см. `creating_maps.md`.
...
- Если вы хотите настроить постоянные характеристики игрока (например, максимальное здоровье), то унаследуйтесь от `game/player/player.tscn`, поменяйте что нужно, сохраните новую сцену в папку Вашего события и добавьте её (перед этим можно удалить сцену по умолчанию, если она Вам не нужна) в массив `Entity Scenes` на узле события. Затем Вы можете к ней обращаться в `_get_player_scene` (если Вы не перезаписали сцену по умолчанию, иначе этот метод и без переопределения вернёт нужную сцену).
...
В первую очередь, в сцене события добавьте нужные узлы интерфейса в качестве дочерних узла `UI/Main`. Затем уже в скрипте интерфейса события напишите нужные методы. RPC в скрипте интерфейса использовать не рекомендуется. После этого из скрипта своего события вызывайте методы по необходимости.

Также при рисовании элементов интерфейса не забывайте про [`drawing.md`](./drawing.md).

## Параметры события

Во вкладке `Parameters` можно найти словарь параметров события типа `String` - `EventParameter`, где ключ - ID параметра. Сами параметры - это значения `int`, в которых можно хранить время матча, количество раундов для победы и многое другое.

В ресурсах типа `EventParameter` хранится информация о параметре: имя, значение по умолчанию, тип параметра (точнее, как он будет визуально представлен админу), максимальное и минимальное значения, а также его шаг. Можно настроить визуально и суффикс, и префикс параметра. Необходимо ещё и назначить иконку параметра размером 80 на 80. Рекомендуется чёрно-белый стиль, как и у остальных иконок в игре.

В коде самого события можно обращаться к параметрам через поле `parameters` типа `Dictionary[String, int]`, где ключ - ранее указанный в `EventData` ID параметра, а значение - непосредственно сам установленный админом параметр.
