...
- Названия групп и анимаций должны быть в `snake_case`.
- Если появление объекта должно быть синхронизовано через `MultiplayerSpawner`, обязательно указывайте `force_readable_name` как `true` в методе `add_child` при добавлении этого объекта.
- Не рекомендуется использовать `SceneTreeTimer` в игровой части, так как он не подчиняется `Process Mode` и может продолжать работать после освобождения узла-владельца.
- Используйте `preload()` только в скриптах, которые могут существовать без сцены. В остальных случаях ограничьтесь связкой `load()` и `ResourcePreloader`.
