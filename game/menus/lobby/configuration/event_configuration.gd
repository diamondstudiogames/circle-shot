	...
	for parameter_id: String in Globals.items_db.events[_lobby.selected_event].parameters:
		...
		match parameter.type:
			EventParameter.Type.CHECK_BUTTON:
				...
				check_button.toggled.connect(_on_check_button_toggled.bind(parameter_id))
			EventParameter.Type.SLIDER:
				...
				slider.value_changed.connect(
						_on_slider_value_changed.bind(parameter_id, value_label))
...
func _save_and_close() -> void:
	if _lobby.is_admin():
		_lobby.request_set_environment.rpc_id(MultiplayerPeer.TARGET_PEER_SERVER,
				_lobby.selected_event, _lobby.selected_map, _lobby.selected_event_parameters)
	hide()


func _on_spin_box_value_changed(value: float, parameter_id: String) -> void:
	_lobby.selected_event_parameters[parameter_id] = int(value)


func _on_slider_value_changed(value: float, parameter_id: String, value_label: Label) -> void:
	_lobby.selected_event_parameters[parameter_id] = int(value)
	value_label.text = Globals.items_db.events[_lobby.selected_event].parameters[parameter_id] \
			.get_parameter_as_string(int(value))


func _on_check_button_toggled(toggled_on: bool, parameter_id: String) -> void:
